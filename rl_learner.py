...
import sample_wrapper


def load_data(filename="./data.csv", max_size=float('inf'), console=True):
    data_set = sample_wrapper.DodgeDataset.from_csv((30, 2, 30, 2), filename=filename)
...
def sample_game(actor, time_sec, n_parallel, overwrite=True, console=True, filename="./data.csv"):
    game = dodge_parallel.DodgeParallel(n_parallel)
    wrap = sample_wrapper.SampleWrapper(game)
